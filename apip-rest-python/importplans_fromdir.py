...
def main(*mainargs):
    ...
    if len(mainargs) == 0 : #if len(sys.argv)>1 and len(sys.argv)<5:
        ...
        parser = argparse.ArgumentParser(description='Import Plans into APICS by reading all json files in specified dirpath;')
        ...
        #destdir = './'
        logging.info('Directory used for importplans= '+apidirpath)
    ...
    for jsonfile in findalljsonfiles(apidirpath): #Loop through all json files in CWD and import them
        ...
        apisvr.importplanX(jsonfile) #apisvr.importapi_stripartifacts(jsonfile) #todo use importapi when ready
