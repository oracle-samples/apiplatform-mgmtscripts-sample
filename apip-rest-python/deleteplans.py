...
def main(*mainargs):
    ...
    apiid= None
    if len(mainargs) == 0 : 
        parser = argparse.ArgumentParser(description='Deletes a Plan from APICS by providing its api-id; When no Plan ID is provided deletes all Plans and can be used to cleanup')
        ...
        parser.add_argument('--planid', dest='apiid', help='Planid to be deleted from API Platform server, warning - deletes all API when omitted', required=False)
